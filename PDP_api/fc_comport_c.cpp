...
// Host Commands
#include "fc_command_c.h" // for data types and FC_COM_WR_CONTINUE
...
// Replaced by user for the target embedded processor.
// access to UART drivers
// same as usb_vcp

// Replaced by user for the target embedded processor.
// access to usb-i2c drivers
#include "usb_i2c.h" // for usb-i2c driver
extern USB_I2C *usbI2C; // for usb-i2c driver

// Replaced by user for the target embedded processor.
// access to usb-spi drivers
#include "usb_spi.h" // for usb-spi driver
extern USB_SPI *usbSPI; // for usb-spi driver
...
bool Host_DeviceRead(uint32 numBytesToRead, uint8* buf)
{
    ...
    else if(usbI2C->enabled)
        return usbI2C->I2C_DeviceRead(numBytesToRead, buf);
    else if(usbSPI->enabled)
        return usbSPI->SPI_DeviceRead((int)numBytesToRead, buf);
    return 0;
}
//
bool Host_DeviceWrite(uint8 numBytesToWrite, uint8* buf)
{
    if(usbVCP->enabled)
        ...
        return usbI2C->I2C_DeviceWrite(numBytesToWrite, buf);
    else if(usbSPI->enabled)
        return usbSPI->SPI_DeviceWrite((int)numBytesToWrite, buf);
    return 0;
}
//
bool Host_WaitForDeviceAck(uint8 timeoutSeconds)
{
    if(usbVCP->enabled)
        ...
        return usbI2C->I2C_WaitForAck(timeoutSeconds);
    else if(usbSPI->enabled)
        return usbSPI->SPI_WaitForDeviceAck(timeoutSeconds);
    return 0;
}
//
//
...
The txbuf[] contains:
1. 1 byte: size - size of tx packet, including command byte and including size byte
