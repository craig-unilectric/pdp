...
#include "pdp_current_c.h" // for data types and structs
...
ColorWindowLUT cwlut[256] = {}; // in fc_command_c.h

// in fc_command_c.h
FeatureList fl; // allocates list sized for MAX_FEAT features
RegionList rl; // allocates list sized for MAX_FEAT features
...
Slow camera routines.
IoTcam sends late acks for:
FC_COM_WR_CAMERA_TEST_PATTERN
FC_COM_WR_RGB565_160x120
FC_COM_WR_RGBJPEG_640x480
...
// camera power
//
//#define FC_COM_WR_CAMERA_POWER_DOWN		0x90
//#define FC_COM_WR_CAMERA_POWER_UP		0x91

/*
void Host::camera_power(void)
{
    //
    FC_stop();
    //
    if(ui->pushButton_camera_power->isChecked()){
        ui->pushButton_camera_power->setText("Camera Power Off");
        FC_writeCommand(FC_COM_WR_CAMERA_POWER_DOWN);
        Log("Camera power off.");
    }
    else{
        ui->pushButton_camera_power->setText("Camera Power On");
        FC_writeCommand(FC_COM_WR_CAMERA_POWER_UP);
        Log("Camera power on.");
    }
    //
    FC_run();
...
/*
#define FC_COM_WR_ACCEL_WRREG 	0x94
#define FC_COM_WR_ACCEL_RDREG 	0x95
#define FC_COM_RD_ACCEL_REG 		0x96
*/
...
uint32 FC_capture_JPG_image(uint8* buf, uint32 bufsize)
{
    ...
    /*
    // Optional, end a bad jpeg to recover the image.
    // Does not work for Qt::Image.
    else{
        buf[i - 2] = 0xff;
        buf[i - 1] = 0xd9;
        jpgsize = bufsize;
...
#define FC_COM_RD_SD_GOODREAD	0xab
    ...
    uint32        	SDimageBlockCnt;
    uint32        	SDimageCounter;
    uint32        	SDimageMax;
*/
